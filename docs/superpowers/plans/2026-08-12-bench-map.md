...
Run the harness **three times**, not once. One run cannot show how much a number moves on its own, and design spec section 2.1 measured that several of them move. `byblos-bench baseline` refuses fewer than three inputs.

```bash
for i in 1 2 3; do
  CGO_ENABLED=0 go run ./cmd/byblos-bench run -set ./bench-v1 -out /tmp/r$i.json
done
CGO_ENABLED=0 go run ./cmd/byblos-bench baseline \
  -runs /tmp/r1.json,/tmp/r2.json,/tmp/r3.json \
  -out internal/bench/baseline.json \
  -commit "$(git rev-parse HEAD)" \
  -benchset "$(cut -d' ' -f1 bench-v1.tar.zst.sha256)" \
  -harness "$(cat cmd/byblos-bench/*.go internal/bench/map.go | shasum -a 256 | cut -d' ' -f1)"
```

**Then prove the baseline before committing it.** Run the harness a fourth time and score that unchanged run against the new baseline. It must FAIL with a score near zero. If it passes, the recorded spreads are too narrow for this corpus and the baseline would let jitter through:

```bash
CGO_ENABLED=0 go run ./cmd/byblos-bench run -set ./bench-v1 -out /tmp/r4.json
CGO_ENABLED=0 go run ./cmd/byblos-bench score \
  -baseline internal/bench/baseline.json -head /tmp/r4.json   # must exit 1
```
