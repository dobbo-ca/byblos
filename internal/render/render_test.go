...
func render100(t *testing.T, src string) *image.RGBA {
	t.Helper()
	img, err := Page(context.Background(), []byte(src), box100, 1, nil)
	if err != nil {
		t.Fatalf("Page: %v", err)
	}
...
func TestRasterDimensionsClamped(t *testing.T) {
	for _, tc := range []struct {
		...
		if _, err := Page(context.Background(), nil, tc.box, tc.scale, nil); err == nil {
...
func TestPathPointBudget(t *testing.T) {
	...
	if _, err := Page(context.Background(), []byte(src), box100, 1, nil); err == nil {
...
func TestFillWorkBudget(t *testing.T) {
	...
	b.WriteString("f")
	if _, err := Page(context.Background(), []byte(b.String()), box100, 1, nil); err == nil {
...
func TestContextCancellation(t *testing.T) {
	...
	if _, err := Page(ctx, []byte(src), box100, 1, nil); err == nil {
...
func TestFillPixelWorkBudget(t *testing.T) {
	defer func(v int64) { maxFillWork = v }(maxFillWork)
	maxFillWork = 350
	if _, err := Page(context.Background(), []byte("0 g 0 0 100 100 re f"), box100, 1, nil); err == nil {
...
func TestStrokeWorkBudget(t *testing.T) {
	defer func(v int64) { maxFillWork = v }(maxFillWork)
	maxFillWork = 16
	if _, err := Page(context.Background(), []byte("0 g 4 w 10 10 m 90 90 l S"), box100, 1, nil); err == nil {
...
func TestMalformedStreamStillReturnsCanvas(t *testing.T) {
	img, err := Page(context.Background(), []byte("nonsense ops here 1 0 0 rg 10 20 30 40 re f"), box100, 1, nil)
	if err != nil {
		t.Fatalf("Page: %v", err)
	}
	assertRect(t, img, 10, 40, 40, 80, red)
