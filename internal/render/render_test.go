...
func render100(t *testing.T, src string) *image.RGBA {
	t.Helper()
	img, err := Page(context.Background(), []byte(src), box100, 1)
	if err != nil {
		t.Fatalf("Page: %v", err)
	}
...
func TestRasterDimensionsClamped(t *testing.T) {
	for _, tc := range []struct {
		...
		if _, err := Page(context.Background(), nil, tc.box, tc.scale); err == nil {
...
func TestPathPointBudget(t *testing.T) {
	...
	if _, err := Page(context.Background(), []byte(src), box100, 1); err == nil {
...
func TestFillWorkBudget(t *testing.T) {
	...
	b.WriteString("f")
	if _, err := Page(context.Background(), []byte(b.String()), box100, 1); err == nil {
...
func TestMalformedStreamStillReturnsCanvas(t *testing.T) {
	img, err := Page(context.Background(), []byte("nonsense ops here 1 0 0 rg 10 20 30 40 re f"), box100, 1)
	if err != nil {
		t.Fatalf("Page: %v", err)
	}
	assertRect(t, img, 10, 40, 40, 80, red)
